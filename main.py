...
pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files (x86)\Tesseract-OCR\tesseract.exe"#ocr file
image = cv2.imread('car3.jpg+8
                   5')

image = imutils.resize(image, width=500)#sizing

cv2.imshow("Original Image", image)#python lo image operations all read,write image print
...
s = time.asctime( time.localtime(time.time()))+'        '+'       '+ Final_Text
raw_data={'':[s]}

df = pd.DataFrame(raw_data)
df.to_csv('data.csv',index=False,mode='a')#append
